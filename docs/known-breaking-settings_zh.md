...
| `net.ipv4.conf.all.rp_filter=1`（严格模式） | 透明代理（sing-box TUN）流量被丢弃 | 反向路径过滤太严，代理场景必须松散 | 保留——`proxy.nix` 检测到代理时自动覆盖为 2 | Kicksecure | 开代理后出口流量测试 |
...
| sing-box `tun-in.exclude_uid_range`里两个 UID 相邻 | `sing-box.service` 崩溃循环:`FATAL start service: post-start inbound/tun[tun-in]: auto-redirect: setup nftables: flush nftables: conn.Receive: netlink receive: file exists`。就算把 `endpointsFile`/`outboundsFile` 都临时清空(零 endpoint、零 outbound)照样必现,以此排除了是它们的问题 | `sing-tun` 的 `redirect_nftables_rules.go` 在 `len(ExcludeUID) > 1` 时,把每段 UID 区间编码成两个 set element(`Start` 和标了 `IntervalEnd` 的 `End+1`)。两段首尾相接(前一段的 `End+1` 正好等于后一段的 `Start`)就会产生两个 key 相同的 element,nftables 拒绝重复(EEXIST)。排除 unbound(uid 977)和 dnscrypt-proxy(uid 976,当初特意选的挨着)正好撞上。跟"区间碰地址空间顶端"那个已知 bug(这仓库现在 pin 的 `sing-tun` 版本已经通过 [SagerNet/sing-tun#83](https://github.com/SagerNet/sing-tun/pull/83) 修过)是同一类区间编码问题,但是另一条没被那次补丁覆盖到的代码路径 | 已修复——dnscrypt-proxy 的 uid 从 976 改成 970,不再挨着 unbound 的 977 | 本仓库自己选 uid 时踩上了 `sing-tun` 地址集构建逻辑的坑 | `journalctl -u sing-box`:"started at tun0" 之后跟着同样的 FATAL;把 `exclude_uid_range` 里两个 uid 设成相邻、`auto_redirect=true` 即可复现 |
...
| `init_on_free=1` 内核参数 | 没在这台机器上实测——凭社区文档里记录的分配器开销(不只清零分配出去的页，连释放的页也清零)对高频分配/释放场景(游戏引擎、GC 密集运行时)的冲击比只开 `init_on_alloc=1` 更大而按下不加 | KSPP 标准搭档,配 `init_on_alloc=1`(已保留)——free 侧清零历来是这对参数里更贵的那一半 | 未采纳——只保留 `init_on_alloc=1`；除非在 omen15 上真的跑一次前后帧率对比,否则不重新考虑 | nix-mineral / tails（2026-09-25 上游审查） | 未实测——需要一次可控的、分配密集场景的帧率基准测试 |
| `hardened_usercopy=1` 内核参数 | 没实测——凭每次 `copy_to_user`/`copy_from_user` 系统调用边界多一次边界检查的开销而按下不加 | KSPP 推荐的加固项，这颗 cachyos-bore-lto 内核没有默认编译开启 | 未采纳；除非有真实的系统调用密集型基准测试，否则不重新考虑 | nix-mineral（2026-09-25 上游审查） | 未实测 |
| `pti=on` 内核参数 | 没实测——按下不加是因为会覆盖掉这颗 AMD CPU 本来就有的自动豁免。`mitigations=auto` 本来就正确地不开 PTI（AMD 不受 Meltdown 影响）；强制 `pti=on` 图个"多一层保险"等于主动放弃这个真实的性能优势换取边际安全收益，而 PTI 的开销历来恰恰在系统调用/上下文切换密集的场景（比如游戏）最大 | nix-mineral 无条件应用 `pti=on`，哪怕在不受影响的 CPU 上也一样 | 未采纳——保留 AMD 的 `mitigations=auto` 豁免不动 | nix-mineral（2026-09-25 上游审查） | 未实测 |
| `security.doas.extraRules` wheel 规则的 `persist = true` | 不适用——不是 bug，是刻意的便利性/安全性取舍。OpenBSD 自己的 `doas.conf` 示例是 `permit keepenv :wheel`，不带 `persist`，也就是每次调用都重新要密码，哪怕同一个 shell 里隔几秒钟连续用 | `persist = true` 会把成功验证的结果缓存一段宽限期（类似 sudo 的 timestamp 行为）——比 OpenBSD 自己的默认更方便，但也更弱；在个人可信硬件的机队上，OpenBSD 那种威胁模型不明显适用 | 保留现状，图方便——写在这里是为了下次审查这份文档时看出这是权衡过的选择，不是漏改 | OpenBSD（2026-09-25 上游审查） | 不适用 |

| `networking.networkmanager.settings.connection."dhcp-send-hostname"` | 不会崩溃，不会导致 build 失败——就是静默被忽略。NetworkManager 只认带协议前缀的 key 名，裸的 `connection.dhcp-send-hostname` 根本不是 NM 认识的设置，这行代码加上去之后大概率就一直是空操作——意味着这条隐私设置表面上在，实际主机名可能一直在通过 DHCP 广播出去 | 查 NM 的 `nmcli`/keyfile 文档确认：真正的 key 是 `connection.ipv4.dhcp-send-hostname` 和 `connection.ipv6.dhcp-send-hostname`（按协议分开，前缀必须带）——`[connection]` 下裸的 `dhcp-send-hostname` 不是 NM 会解析的东西 | 已修复——拆成两个带正确前缀的 key（`privacy.nix`） | secureblue（2026-09-25 上游审查）——secureblue 自己的脚本是把 `ipv4.` 和 `ipv6.` 两个变体一起设的，正是这一点带出了这里漏前缀的问题 | 上线后 `nmcli -f 802-3-ethernet,ipv4,ipv6 connection show <profile> \| grep dhcp-send-hostname` 应该能看到两个带前缀的 key 都是 `no`；真正的证据是抓包看 DHCP 续租请求（Option 12 / 请求里没有主机名） |
| `passwd.rules.password.unix.settings.rounds = 65536` | 这次季度复查前一直没测过——`passwd`/`doas passwd` 改密码这条路径本该是坏的。已经实测复现(不是只凭文档推断)：`mkpasswd -m yescrypt -R 65536 -- x` → `crypt_gensalt: Invalid argument`，退出码 2。`-R 8`、`-R 11` 都能成功；`-R 5000` 同样报错 | `ENCRYPT_METHOD` 默认是 `YESCRYPT`（nixpkgs `shadow.nix`），pam_unix 的 `rounds=` 参数对 yescrypt 也生效（`pam_unix(8)` 确认）——但对 yescrypt 来说这不是轮数，是 cost factor，硬性范围 `1-11`（`login.defs(5)` 的 `YESCRYPT_COST_FACTOR` 一节确认）。`65536` 是 SHA-crypt 量级的值，对 yescrypt 从来就不合法；libxcrypt 的 `crypt_gensalt()` 直接拒绝，不会静默截断 | 已修复——改成 `8`，跟新加的 `security.loginDefs.settings.YESCRYPT_COST_FACTOR = 8` 保持一致 | 本仓库自己的 `security.nix`（这个值的来源不清楚——大概率是从 SHA-crypt 时代的加固参考里照抄，没针对 yescrypt 调整） | `mkpasswd -m yescrypt -R <n> -- x`——n 在 1-11 内退出码 0 并产出 `$y$...` 哈希，超出范围报 `crypt_gensalt: Invalid argument` 退出码 2；上线后记得真的跑一次 `passwd`/`doas passwd` 确认改密码成功，不能只看 build 过了 |

| `pam_faillock` 账号锁定 + `pwquality` 密码强度检查 | 不适用——没试过，是权衡后主动否掉的决定，不是踩坑 | 这个机队 SSH 已经全走 key（`PasswordAuthentication=false`），两个机制要防的爆破/弱密码面在远程这条路上本来就接近零；`faillock` 真正的风险是单人管理的机器打错密码把自己锁外面——oci 尤其糟，是无头远程机，锁死了只能靠 Oracle Cloud 的 VNC 控制台，没法快速重试 | 未采纳 | kicksecure / secureblue（2026-09-25 上游审查） | 不适用 |
| oci：nftables `synproxy` + 按源 IP `connlimit`（SYN flood / 连接数上限加固） | 不适用——没试过，权衡后选择维持现状 | `net.ipv4.tcp_syncookies=1`（`security.nix:95`）已经零成本覆盖了这条要加的核心威胁——SYN flood 打爆 backlog 队列；`crowdsec-firewall-bouncer` 也已经在做 IP 级别封禁。上 nftables 版本意味着把 oci 的防火墙后端整个从 `networking.firewall`（iptables）迁走，有真实风险打断 `crowdsec-firewall-bouncer` 插规则（大概率是照着 iptables 写的），`connlimit` 也可能误伤真实 Ente 客户端的多并发连接——已有覆盖的基础上不值这个成本 | 未采纳——保持 `networking.firewall`（iptables），NixOS 自带的临时防火墙操作命令继续能用 | grapheneos-infra（2026-09-25 上游审查） | 不适用 |

| `iw-set-regdomain`（按时区自动设 WiFi 监管域） | 目前在所有主机上都是永久空操作——实测确认：跑一下编译出来的脚本，`journalctl -t iw-set-regdomain` 显示 `Timezone is UTC, with no associated country. Not setting wireless regulatory domain.` | `core.nix:104` 全队默认 `time.timeZone = lib.mkDefault "UTC"`，没有主机覆盖它。脚本（照 Pop!_OS 原版搬的，只改了 tzdata 的 nix store 路径而不是原版假设的 `/usr/share/zoneinfo/`）从 `/etc/localtime` 反查国家，遇到 UTC/GMT 等本来就设计成"无对应国家"——这是设计如此,不是 bug | 保留——`systemd.paths` 盯着 `/etc/localtime`，一旦有人跑 `timedatectl set-timezone <Zone>`（比如出国时）就会正常触发；只是在那之前一直不干活 | pop-default-settings（2026-09-25 上游审查） | `/etc/localtime` 变化后或开机时看 `journalctl -t iw-set-regdomain` |
