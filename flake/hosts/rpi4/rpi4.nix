...
{ config, lib, pkgs, inputs, ... }: {
  ...
  config = {
    ...
    boot.kernel.sysctl = {
      ...
      # 尝试稍微减小队列长度，避免堆积
      "net.core.netdev_max_backlog" = lib.mkForce 1024; # 默认通常是 1000
    ...
    systemd.services.network-optimization = {
      ...
      serviceConfig = {
        ...
        ExecStart = pkgs.writeShellScript "optimize-network" ''
          ...
          ${pkgs.ethtool}/bin/ethtool -K end0 gso off gro off tso off
          # 1. 清除旧规则
          ${pkgs.iproute2}/bin/tc qdisc del dev end0 root 2>/dev/null || true
          # 2. 应用新规则：
          #    bandwidth 30mbit: 你之前的测试值，稳定后可慢慢往上加
          ${pkgs.iproute2}/bin/tc qdisc add dev end0 root cake besteffort nat
