  ...
  options.modules.networking.sysfsTuning = {
    ...
    interfaces = lib.mkOption {
      type = lib.types.attrsOf (
        lib.types.submodule {
          options = {
            rps_cpus = lib.mkOption {
              type = lib.types.nullOr lib.types.str;
              default = null;
            };
            xps_cpus = lib.mkOption {
              type = lib.types.nullOr lib.types.str;
              default = null;
            };
          };
    ...
    boot.kernel.sysfs =
      let
        tuneIface =
          ...
          let
            rx =
              if tunings.rps_cpus != null then
                lib.setAttrByPath [ "class" "net" iface "queues" "rx-0" "rps_cpus" ] tunings.rps_cpus
              else
                { };
            tx =
              if tunings.xps_cpus != null then
                lib.setAttrByPath [ "class" "net" iface "queues" "tx-0" "xps_cpus" ] tunings.xps_cpus
              else
                { };
          in
